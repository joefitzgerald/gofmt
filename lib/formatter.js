...
class Formatter {
  ...
  handleCommands () {
    ...
    atom.commands.add('atom-workspace', 'golang:gofmt', () => {
      if (!this.ready() || !this.getEditor()) {
        return
      }
      this.format(this.getEditor(), 'gofmt')
    })
    atom.commands.add('atom-workspace', 'golang:goimports', () => {
      if (!this.ready() || !this.getEditor()) {
        return
      }
      this.format(this.getEditor(), 'goimports')
    })
    atom.commands.add('atom-workspace', 'golang:goreturns', () => {
      if (!this.ready() || !this.getEditor()) {
        return
      }
  ...
  observeConfig () {
    ...
    this.subscriptions.add(atom.config.observe('gofmt.formatArgs', (formatArgs) => {
  ...
  subscribeToSaveEvents () {
    this.saveSubscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      bufferSubscriptions.add(editor.getBuffer().onWillSave((filePath) => {
        this.format(editor, this.formatTool, filePath)
      }))
  ...
  format (editor = this.getEditor(), tool = this.formatTool, filePath) {
    ...
    let args = this.formatArgs.slice()
    if (args.indexOf('-w') !== -1) {
